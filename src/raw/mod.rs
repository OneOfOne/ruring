#![allow(non_upper_case_globals, non_camel_case_types, non_snake_case, clippy::missing_safety_doc, unused)]
...
macro_rules! io_uring_smp_load_acquire {
	($ptr:expr) => {{
		let x = std::ptr::read_volatile($ptr);
		io_uring_barrier!();
		x
	}};
}
...
pub unsafe fn io_uring_prep_rw(op: c_uint, sqe: *mut io_uring_sqe, fd: c_int, addr: *const c_void, len: c_uint, offset: off_t) {
	(*sqe).opcode = op as u8;
	(*sqe).flags = 0;
	(*sqe).ioprio = 0;
	(*sqe).fd = fd;
	...
	(*sqe).personality = 0;
	(*sqe).__bindgen_anon_5.splice_fd_in = 0;
	(*sqe).__bindgen_anon_6.bindgen_union_field = [0, 0];
}

pub unsafe fn io_uring_prep_readv(sqe: *mut io_uring_sqe, fd: c_int, iovecs: *const libc::iovec, nr_vecs: u32, offset: off_t) {
	io_uring_prep_rw(IORING_OP_READV, sqe, fd, transmute(iovecs), nr_vecs, offset);
}

pub unsafe fn io_uring_prep_read_fixed(sqe: *mut io_uring_sqe, fd: i32, buf: *mut c_void, nbytes: u32, offset: off_t, buf_index: c_ushort) {
	io_uring_prep_rw(IORING_OP_READ_FIXED, sqe, fd, buf, nbytes, offset);
	(*sqe).__bindgen_anon_4.buf_index = buf_index;
}

pub unsafe fn io_uring_prep_writev(sqe: *mut io_uring_sqe, fd: i32, iovecs: *const libc::iovec, nr_vecs: u32, offset: off_t) {
	io_uring_prep_rw(IORING_OP_WRITEV, sqe, fd, transmute(iovecs), nr_vecs, offset);
}

pub unsafe fn io_uring_prep_write_fixed(sqe: *mut io_uring_sqe, fd: i32, buf: *mut c_void, nbytes: u32, offset: off_t, buf_index: u16) {
...
pub unsafe fn io_uring_prep_sendmsg(sqe: *mut io_uring_sqe, fd: i32, msg: *const c_void, flags: u32) {
...
pub unsafe fn io_uring_prep_timeout(sqe: *mut io_uring_sqe, ts: *mut __kernel_timespec, count: off_t) {
...
pub unsafe fn io_uring_sq_space_left(ring: *const io_uring) -> u32 {
	(*ring).sq.kring_entries as u32 - ((*ring).sq.sqe_tail - (*ring).sq.sqe_head)
}
...
unsafe fn __io_uring_peek_cqe(ring: *mut io_uring, cqe_ptr: *mut *mut io_uring_cqe) -> c_int {
	...
	loop {
		...
		if !cqe.is_null() && (*cqe).user_data == LIBURING_UDATA_TIMEOUT {
			if (*cqe).res < 0 {
				err = (*cqe).res;
			}
...
pub unsafe fn io_uring_peek_cqe(ring: *mut io_uring, cqe_ptr: *mut *mut io_uring_cqe) -> c_int {
	...
	__io_uring_get_cqe(ring, cqe_ptr, 0, 0, std::ptr::null_mut())
}
...
pub unsafe fn io_uring_wait_cqe(ring: *mut io_uring, cqe_ptr: *mut *mut io_uring_cqe) -> c_int {
	...
	__io_uring_get_cqe(ring, cqe_ptr, 0, 1, std::ptr::null_mut())
}
